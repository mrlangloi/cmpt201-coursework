...
int main() {
  ...
  int key_fd = open("public_key.pem", O_RDONLY, S_IRUSR);
  FILE *key_file = fdopen(key_fd, "r");
  if (key_file == NULL) {
    handle_error("fdopen");
  }
...
#define MAX_FILE_SIZE 512
  ...
  size_t message_bytes = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  if (message_bytes < 1) {
    handle_error("read_all_bytes: message_path");
  }

  size_t sign_bytes = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  if (sign_bytes < 1) {
    handle_error("read_all_bytes: sign_path");
  }
  ...
  ret = EVP_DigestVerifyUpdate(ctx, message, message_bytes);
  if (ret <= 0) {
    handle_error("EVP_DigestVerifyUpdate");
  }

  ret = EVP_DigestVerifyFinal(ctx, signature, sign_bytes);

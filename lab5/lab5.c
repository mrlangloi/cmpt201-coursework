 ...
 *      if the block at the current address + current size (i.e. the next block)
 * is free: merge with next block update current size continue proceed to the
 * next block, current block = current address + current size
 *
 * this algorithm should merge free blocks that are adjacent to each other
 * although it traverses through used blocks, it does not alter any of their
 * data
 */
